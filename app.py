...
from helpers import user_average_rating, user_completed_matches, user_points, user_badges, unread_notifications_count, add_notification, skill_match_score

load_dotenv()

app = Flask(__name__)
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'dev-secret-change-me')
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///skill_exchange.db').replace('postgres://', 'postgresql://')
...
def index():
    ...
    return render_template('index.html', stats=stats, popular_skills=popular_skills, top_users=top_users)
...
def profile():
    if request.method == 'POST':
        ...
        if not current_user.name:
            ...
            db.session.commit(); flash('個人資料已更新。','success')
...
def skills():
    ...
    if keyword:
        query = query.filter(or_(Skill.title.contains(keyword), Skill.description.contains(keyword)))
    if category_id:
    ...
    return render_template('skills.html', skills=skills, categories=categories, keyword=keyword, category_id=category_id, method=method)
...
def add_skill():
    ...
    if request.method == 'POST':
        ...
        if not skill.title or not skill.description:
            ...
            db.session.add(skill); db.session.commit()
            flash('技能已上架。','success')
...
def match_center():
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            if skill.user_id == current_user.id:
                ...
                exists = Match.query.filter(Match.skill_id==skill.id, Match.requester_id==current_user.id, Match.status.in_(['pending','accepted'])).first()
                if exists:
                    flash('你已送出過這筆媒合邀請。','error')
                else:
                    m = Match(skill_id=skill.id, requester_id=current_user.id, receiver_id=skill.user_id, message=request.form.get('message') or '想和你進一步交換技能。')
        ...
        if action in ['accepted','rejected','completed','cancelled']:
            ...
            if current_user.id not in [m.requester_id, m.receiver_id]: abort(403)
            m.status = action; db.session.commit()
    ...
    matches = Match.query.filter(or_(Match.requester_id==current_user.id, Match.receiver_id==current_user.id)).order_by(Match.updated_at.desc()).all()
...
def chat(match_id):
    ...
    if request.method == 'POST':
        ...
        if content:
            db.session.add(Message(match_id=m.id, sender_id=current_user.id, receiver_id=other_id, content=content))
            db.session.commit(); add_notification(other_id, 'message', '你收到一則新訊息。', m.id)
...
@login_required
def review():
    completed = Match.query.filter(Match.status=='completed', or_(Match.requester_id==current_user.id, Match.receiver_id==current_user.id)).all()
    if request.method == 'POST':
        m = Match.query.get_or_404(int(request.form.get('match_id')))
        if current_user.id not in [m.requester_id, m.receiver_id]: abort(403)
        ...
        db.session.add(Review(match_id=m.id, reviewer_id=current_user.id, reviewee_id=reviewee_id, rating=rating, comment=request.form.get('comment','').strip()))
        db.session.commit(); add_notification(reviewee_id, 'review', '你收到新的評價。', m.id)
        flash('評價已送出。','success')
